...
export type Store = { listProducts(): Promise<Product[]>; getProduct(slug: string): Promise<Product | undefined>; upsertProduct(product: Product): Promise<Product>; listOrders(): Promise<Order[]>; createOrder(input: { email: string; items: CartItemInput[] }): Promise<Order>; markOrderPaid(orderId: string): Promise<Order | undefined>; };
